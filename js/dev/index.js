      ...
      */
      ...
      on: {
        init(swiper) {
          ...
          if (bp >= 992) swiper.autoplay.stop();
        },
        breakpoint(swiper) {
          const bp = Number(swiper.currentBreakpoint);
          if (bp >= 992) swiper.autoplay.stop();
...
var MatchMedia = /* @__PURE__ */ function() {
  ...
  _proto6.add = function add(conditions, func, scope) {
    ...
    var context3 = new Context(0, scope || this.scope), cond = context3.conditions = {}, mq2, p, active;
    ...
    for (p in conditions) {
      if (p === "all") {
        ...
        mq2 = _win$3.matchMedia(conditions[p]);
        if (mq2) {
          _media.indexOf(context3) < 0 && _media.push(context3);
          (cond[p] = mq2.matches) && (active = 1);
          mq2.addListener ? mq2.addListener(_onMediaChange) : mq2.addEventListener("change", _onMediaChange);
        }
      }
    }
  ...
  split(config3) {
    ...
    if (splitCharacters || splitWords || splitLines) {
      this.elements.forEach((element, index) => {
        ...
        let chars2 = [], words = [], lines = [], charWrapper = splitCharacters ? _getWrapper("char", config3, chars2) : null, wordWrapper = _getWrapper("word", config3, words), i, curWord, smartWrapSpan, nextSibling;
...
function enableSubmenu() {
  document.querySelectorAll(".menu__list > li").forEach((parentLi) => {
    const items = parentLi.querySelectorAll(".menu__submenu li");
    ...
    const onMouseLeave = () => {
      ...
      items.forEach((li) => li.style.transitionDelay = "");
    };
    parentLi.addEventListener("mouseenter", onMouseEnter);
    parentLi.addEventListener("mouseleave", onMouseLeave);
    handlers.push({ parentLi, onMouseEnter, onMouseLeave });
  });
}
function disableSubmenu() {
  handlers.forEach(({ parentLi, onMouseEnter, onMouseLeave }) => {
    parentLi.removeEventListener("mouseenter", onMouseEnter);
